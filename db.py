...
def listar_reservas(peca_id: int = None, apenas_ativas: bool = False, usuario_id: int = None) -> list[dict]:
    ...
    try:
        ...
        ordenacao = [
            case(
                (
                    Reserva.status == ReservaStatus.PENDENTE,
                    0,
                ),
                (
                    Reserva.status == ReservaStatus.RETIRADO,
                    1,
                ),
                else_=2,
            ),
        ]
        reservas = q.order_by(*ordenacao).all()
        resultado = [_reserva_dict(r) for r in reservas]
        
        # Ordenar cada grupo por data_retirada com direções diferentes
        pendentes = [r for r in resultado if r['status'] == ReservaStatus.PENDENTE]
        retirados = [r for r in resultado if r['status'] == ReservaStatus.RETIRADO]
        devolvidos = [r for r in resultado if r['status'] == ReservaStatus.DEVOLVIDO]
        
        # Separar retirados em atrasados e normais
        retirados_atrasados = [r for r in retirados if reserva_atrasada(r)]
        retirados_normais = [r for r in retirados if not reserva_atrasada(r)]
        
        pendentes.sort(key=lambda r: r['data_retirada'])
        retirados_atrasados.sort(key=lambda r: r['data_retirada'], reverse=True)
        retirados_normais.sort(key=lambda r: r['data_retirada'], reverse=True)
        devolvidos.sort(key=lambda r: r['data_retirada'], reverse=True)
        
        return pendentes + retirados_atrasados + retirados_normais + devolvidos
    finally:
        db.close()
